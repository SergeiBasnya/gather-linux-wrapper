...
function createWindow(): void {
  ...
  const allowedPermissions = new Set([
    ...
    'notifications',
    'fullscreen',
    // clipboard-read : nécessaire pour coller une image (Ctrl+V) dans le chat,
    // Gather lit le presse-papier via navigator.clipboard.read() dans ce flow.
    'clipboard-read',
  ...
  // `will-redirect` couvre les redirections HTTP 3xx (ex. OAuth qui pivote
  // vers une origine non whitelistée) que `will-navigate` ne voit pas.
  // On filtre sur `isMainFrame` : les iframes utilisées par Gather pour les
  // uploads d'images (S3 / GCS pre-signed URLs, redirections vers buckets)
  // ne doivent PAS être bloquées, sinon l'envoi de photos casse.
  const blockOffOriginNavigation = (
  ...
  const template: MenuItemConstructorOptions[] = [
    ...
    },
    {
      ...
      submenu: [
        ...
        // DevTools uniquement en dev : en build packagé, l'accès permettrait
        // d'extraire les cookies de session via document.cookie en quelques sec.
        ...(app.isPackaged
          ? []
          : [{ role: 'toggleDevTools' as const, label: 'Toggle DevTools' }]),
        { type: 'separator' },
        { role: 'resetZoom' },
        { role: 'zoomIn' },
        { role: 'zoomOut' },
